...
export class GeolocaPage implements OnInit {
  ...
  async autoCompletado(mapalocal,marcadorlocal){
    ...
    search.addListener('place_changed',function(){
      var p_place=search.gm_accessors_.place.nj.formattedPrediction
      //console.log(search.getPlace().address_components[6].long_name);
      this.place = search.getPlace().geometry.location;
  ...
  calcularRuta(){
    ...
    this.viaje.fin=this.search.gm_accessors_.place.nj.formattedPrediction
    //console.log(this.search.gm_accessors_.place.nj.formattedPrediction)
    var request = {
      origin: {
        lat: this.latitud1,
        lng: this.longitud1
  ...
  async viajero(){
    ...
    this.viaje.estado = "espera"
    console.log(this.viaje.nombre)
    console.log(this.viaje.fin.length)
    console.log(this.viaje.auto)
    if (this.viaje.capasidad == "" || this.viaje.costo == "" || this.viaje.auto == "" || this.viaje.fin.length == undefined) {
      alert("Debe rellenar todos los campos!")
      return
    }else if(this.viaje.costo < 100) {
      alert("Monto incorrecto! Valor minimo: $100")
      return
    }else{
